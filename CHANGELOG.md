...
### v1.22 — August 2026
**Ticker overhaul: layout, the Meme 100, football, MMA, MLB.** First code change on this branch — everything before it was documentation.
- **Section dots move under the league pill and cap at five**, as a sliding window with the lit pip always inside it and the remainder as `+N`. A progress row in the corner of a game card reads as belonging to the game; under the pill it sits with the league it counts, and the scoreboard gets its width back. **The cursor clamp was mandatory, not cosmetic** — the old renderer compared the cursor against the *visible* pip count, so past the cap every pip drew `done` and none drew `on`; at 14 that was rare, at 5 it would have been constant. `setPill` now writes to a label span, because `textContent` on the pill would have deleted the dots on every league change
- **The Meme 100 face is rebuilt around $CSGN vote share as the hero number** — the flagship token feature was leading with price. Share of vote weight is the number nobody else on television can show, and a bar behind each row makes the board readable before anyone reads a label. The bottom band is gone from this face (it carried ~411px of text in a 375px row, the tightest line in the file). **The overlap was arithmetic, not styling:** `.ml-list` declared 58px and held 80px of rows with no `overflow` and the same `z-index` as its neighbours, and above it `.c-top` declared 26px while `.c-sym` ran 30px with **no `line-height` declared anywhere in the dock** — so `min-height:auto` inflated it and pushed the bottom band off *every* card, not just this one. Both bands now budget to exactly 97px and the smoke test asserts the sum. The sparkline mask was retuned for the 267px micro-price case that almost every memecoin renders as, rather than the 210px case it was set for
- **Upcoming football, weeks before it happens: WEEK 0 (Aug 29), CFB WK 1 (Sep 3–7) and NFL WK 1 (Sep 9–14)** as windowed boards that retire themselves once played. **This fixed a silent bug** — `shouldKeepEventToday` discarded every event not on the current broadcast day *after* the fetch, so `dateRangeDays` had no observable effect at all and MMA, F1, NASCAR and both Wimbledon leagues were throwing away the extra days they had just requested. CFB was also missing `groups=80` and getting a limited group rather than all of FBS
- **MMA renders the card, not the bout.** ESPN gives each bout its own event, so a 14-fight night was 14 anonymous rows. Bouts group under their card with a static event rail and pages of three, reusing the golf pattern; the shared sub-rotation timer was generalised so two paged faces can't collide. **The event name was unreachable** — the bout note and the card name were read from one chained expression with the note first, and the note always exists. Weight class, TITLE and MAIN EVENT ride each bout, and a title fight that is also the main event shows both
- **MLB games-back gets a data source; the renderer was never broken.** `gamesBackOf` → `teamRow` was correct and wired end to end, but its only source was `team.standingSummary`, which the *scoreboard* payload generally omits — and every path degraded to `""` silently, so a feature with no data looked identical to a working one. Added a six-hour-cached standings fetch behind the existing reads. **Not verified against a live payload** (outbound fetch is blocked from the authoring environment), so it is built to work either way
- **RIGHT NOW is two lines, evenly spaced.** It was already two lines but lopsided — a 22px tag over a 52px headline, centred, so the spare 15px of the box pooled above and below. The tag now carries real weight and `space-evenly` distributes the leading
- **The game panel fills instead of pooling dead space.** `.rows` was `flex:0 1 auto` against `.gamebox` on `space-between`, so all leftover width collected as one gap — on a pregame CFB card, roughly 550px, nearly half the panel. Rows grow, empty score and logo columns collapse (148px and 64px of guaranteed blank on an upcoming game), and `fitText` gained a grow branch capped at 1.35×
- **AP / CFP rank chips**, with 25 treated as ESPN's unranked sentinel and a reserved slot so ranked and unranked rows still align. **The possession dot no longer jitters** — its slot is always rendered and toggled with `visibility`, instead of shifting both abbreviations ~23px on every change of possession — and it finally **turns red in the red zone**, which the file's own header comment had claimed for months
- Smoke test grows from 90 to **137 checks**, covering the section-dot window, vote shares, standings parsing, MMA card grouping, slate windows, rank parsing and column collapse — all of which had zero coverage
