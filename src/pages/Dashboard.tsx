        ...
        <Card hover={false} className="p-5">
          ...
          {liveAssignedSlot && liveVolumeSOL > 0 && (
            <p className="text-xs text-gray-500 mt-1">
              Calc: {liveVolumeSOL.toFixed(4)} SOL × {(liveAssignedSlot.creatorFees?.feeMultiplier ?? 0).toFixed(6)} = {liveEstimateSOL.toFixed(6)} SOL
            </p>
          )}
                  ...
                  <div key={slot.id} className="border border-white/10 rounded-lg p-3 text-sm">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-[11px] text-gray-500">Multiplier {(slot.creatorFees?.feeMultiplier || 0).toFixed(6)}</p>
      ...
      {slotInfo && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md bg-[#0c0c1a] border border-white/10 rounded-xl p-5">
            ...
            <p className="text-xs text-gray-400">Fee multiplier: {(slotInfo.creatorFees?.feeMultiplier || 0).toFixed(6)} (max 0.0009)</p>
            <p className="text-xs text-gray-400">Estimated streamer payout (SOL): {(slotInfo.creatorFees?.feeOwedSOL || 0).toFixed(6)}</p>
            <p className="text-xs text-gray-500 mt-2">
              Estimate derived from per-minute slot volume updates and a capped fee multiplier based on pump.fun tiers.
            </p>
