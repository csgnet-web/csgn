...
type UserDoc = { username?: string; status?: string; slotLimits?: { maxConcurrentClaims?: number }; phantom?: { walletAddress?: string; verified?: boolean }; twitch?: { twitchUserId?: string; username?: string; verified?: boolean } }
...
export const handler = withHttp(async (event) => {
  ...
  if (!user || user.status !== 'active') throw forbidden('Active CSGN account required')
  if (!user.phantom?.verified || !user.phantom.walletAddress || !user.twitch?.verified || !user.twitch.username || !user.twitch.twitchUserId) throw forbidden('Verified Phantom and Twitch are required')
  ...
  if (claimed.length >= max) throw conflict(`You can claim up to ${max} future/live slots.`, 'claim_limit_reached')
  const now = new Date()
  const twitchUsername = user.twitch.username
  const twitchChannelUrl = `https://www.twitch.tv/${twitchUsername.toLowerCase()}`
  await commitWrites([updateWrite(`slots/${slotId}`, {
    ...
    assignedName: user.username || twitchUsername, twitchUserId: user.twitch.twitchUserId, twitchUsername, twitchChannelUrl, streamUrl: twitchChannelUrl,
    walletAddress: user.phantom.walletAddress, claimedAt: now, updatedAt: now,
  })], transaction)
